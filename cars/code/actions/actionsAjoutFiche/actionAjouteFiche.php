...
if (isset($_POST['validate'])) {
    ...
    if (!empty($_POST['nom']) && !empty($_POST['selectedType']) && !empty($_POST['selectedAnneeSortie'])
        ...
        && !empty($_FILES['image1']) && $_FILES['image1']['error'] == 0) {
        ...
        $insertFicheOnWebsite = $bdd->prepare('INSERT INTO FICHE (ID_CONSTRUCTEUR, ID_TYPE, ID_MODELE, ID_ANNEE_DEBUT, ID_ANNEE_FIN, ID_SEGMENT, NOM_FICHE, RESUME_FICHE, HISTOIRE_FICHE, DATE_AJOUT, ID_UTILISATEUR) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $insertFicheOnWebsite->execute(array($fiche_constructeur, $fiche_type, $fiche_modele, $fiche_annee_sortie, $fiche_annee_fin, $fiche_segment, $fiche_nom, $fiche_resume, $fiche_histoire, $formated_DATETIME, $_SESSION['id']));

        // Récupérer l'ID de la fiche nouvellement insérée
        $getInfosOfThisFicheReq = $bdd->prepare('SELECT ID FROM FICHE WHERE NOM_FICHE = ?');
        ...
        foreach ($images as $index => $imageField) {
            if (isset($_FILES[$imageField]) && $_FILES[$imageField]['error'] == 0) {
                $tmp_file = $_FILES[$imageField]['tmp_name'];
                $stringFiche = $fiche_id . "_" . ($index + 1) . "_" . $fiche_nom . ".jpg";
                ...
                resizeAndFillImage($tmp_file, $nom_destination, 1920, 1080);
                $imageNames[] = $resStringFiche;
            } else {
                $imageNames[] = "";
            }
        }
        ...
        foreach ($imageNames as $index => $imageName) {
            $insertImgFiche->execute([$fiche_id, $imageName, $index + 1]);
        }
